...
function init() {
    ...
    for (let navbutton of navbuttons){
        if (navbutton.textContent === "Wedding") navbutton.setAttribute("aria-current", true);
        else navbutton.setAttribute("aria-current", false);
        navbutton.addEventListener("click", function(e) {
            navbutton.setAttribute("aria-current", true);
            for (let button of navbuttons)if (button.textContent.trim() !== navbutton.textContent.trim()) button.setAttribute("aria-current", false);
            ...
            for (let section of sections){
                ...
                if (selectedSection !== sectionDiv) sectionDiv.setAttribute("style", "display:none");
                else sectionDiv.setAttribute("style", "display:block");
            }
        });
    }
    ...
    function setCanvasSize(ctx, x) {
        if (x.matches) ctx.canvas.setAttribute("style", "width: 280px; height: 140px");
        else ctx.canvas.setAttribute("style", "width: 400px; height: 200px");
    }
    ...
    function drawFrame(frameX, frameY, canvasX, canvasY) {
        ctx.drawImage(spriteSheet, frameX * WIDTH, frameY * HEIGHT, WIDTH, HEIGHT, canvasX, canvasY, SCALED_WIDTH, SCALED_HEIGHT);
    }
    function gameLoop() {
        ...
        setTimeout(()=>{
            if (animate) frameId = window.requestAnimationFrame(gameLoop);
        }, 200);
    }
